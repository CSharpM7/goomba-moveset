...
unsafe extern "C" fn game_attacks4(agent: &mut L2CAgentBase) {
    ...
    if macros::is_excute(agent) {
        macros::ATTACK(agent, 1, 0, Hash40::new("top"), 14.0, 361, 97, 0, 36, 4.6, 0.0, 3.7, 15.5, None, None, None, 1.0, 1.0, *ATTACK_SETOFF_KIND_ON, *ATTACK_LR_CHECK_F, false, 0, 0.0, 0, false, false, false, false, true, *COLLISION_SITUATION_MASK_GA, *COLLISION_CATEGORY_MASK_ALL, *COLLISION_PART_MASK_ALL, false, Hash40::new("collision_attr_normal"), *ATTACK_SOUND_LEVEL_L, *COLLISION_SOUND_ATTR_KICK, *ATTACK_REGION_NONE);
        macros::ATTACK(agent, 2, 0, Hash40::new("top"), 12.0, 361, 97, 0, 36, 3.85, 0.0, 3.8, 8.0, None, None, None, 1.0, 1.0, *ATTACK_SETOFF_KIND_ON, *ATTACK_LR_CHECK_F, false, 0, 0.0, 0, false, false, false, false, true, *COLLISION_SITUATION_MASK_GA, *COLLISION_CATEGORY_MASK_ALL, *COLLISION_PART_MASK_ALL, false, Hash40::new("collision_attr_normal"), *ATTACK_SOUND_LEVEL_M, *COLLISION_SOUND_ATTR_KICK, *ATTACK_REGION_NONE);
    }
...
unsafe extern "C" fn effect_attacks4(agent: &mut L2CAgentBase) {
    ...
    frame(agent.lua_state_agent, 20.0);
    if macros::is_excute(agent) {
        macros::EFFECT_ALPHA(agent, Hash40::new("sys_attack_impact"), Hash40::new("top"), -2, 4, 15, 0, 0, 0, 1.4, 0, 0, 0, 0, 0, 360, true, 0.6);
    }
}
...
unsafe extern "C" fn effect_attacks4charge(agent: &mut L2CAgentBase) {
    ...
    for i in 0..i32::MAX {
        if macros::is_excute(agent) {
            macros::FOOT_EFFECT(agent, Hash40::new("sys_run_smoke"), Hash40::new("top"), -5, 0, 0, 0, 0, 0, 1, 20, 0, 4, 0, 0, 0, false);
        }
        wait(agent.lua_state_agent, 5.0);
        if macros::is_excute(agent) {
            macros::EFFECT(agent, Hash40::new("sys_smash_flash_s"), Hash40::new("footr"), 2, 0, 2, 0, 0, 0, 1, 4, 4, 4, 0, 0, 0, true);
        }
    }
}
*/
...
unsafe extern "C" fn game_attacks4s2(agent: &mut L2CAgentBase) {
    ...
    if macros::is_excute(agent) {
        ArticleModule::remove_exist(agent.module_accessor, FIGHTER_GOOMBA_GENERATE_ARTICLE_ACCESSORIES, ArticleOperationTarget(*ARTICLE_OPE_TARGET_ALL));
        let heal = DamageModule::damage(agent.module_accessor, 0).min(1.5);
        macros::FT_ADD_DAMAGE(agent, -heal);
    }
...
pub fn install(agent: &mut smashline::Agent) {
	...
	agent.acmd("game_attacks4s2", game_attacks4s2, Priority::Default);
	agent.acmd("effect_attacks4s2", effect_attacks4s2, Priority::Default);
	agent.acmd("sound_attacks4s2", sound_attacks4s2, Priority::Default);
	agent.acmd("expression_attacks4s2", expression_attacks4s2, Priority::Default);
