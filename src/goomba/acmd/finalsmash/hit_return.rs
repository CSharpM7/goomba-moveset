use crate::imports::imports_acmd::*;

unsafe extern "C" fn game_finalhit(agent: &mut L2CAgentBase) {
    if macros::is_excute(agent) {
        macros::WHOLE_HIT(agent, *HIT_STATUS_XLU);
    }
}

unsafe extern "C" fn expression_finalhit(agent: &mut L2CAgentBase) {
    if macros::is_excute(agent) {
        ItemModule::set_have_item_visibility(agent.module_accessor, false, 0);
    }
    frame(agent.lua_state_agent, 1.0);
    if macros::is_excute(agent) {
        ControlModule::set_rumble(agent.module_accessor, Hash40::new("rbkind_erase"), 0, false, *BATTLE_OBJECT_ID_INVALID as u32);
    }
    frame(agent.lua_state_agent, 2.0);
    if macros::is_excute(agent) {
        ControlModule::set_rumble(agent.module_accessor, Hash40::new("rbkind_attackll"), 0, false, *BATTLE_OBJECT_ID_INVALID as u32);
    }
}


pub fn install(agent: &mut Agent) {
	agent.acmd("game_finalhit", game_finalhit, Priority::Default);
	agent.acmd("game_finalreturn", game_finalhit, Priority::Default);
	agent.acmd("expression_finalhit", expression_finalhit, Priority::Default);
}